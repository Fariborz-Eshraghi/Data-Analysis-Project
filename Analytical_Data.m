...
I_vals = [0, 2.05, 3.6 ,5, 5.5, 6, 6.25, 6.5, 6.75, 7, 7.5];
...
for I = I_vals
    ...
    [~, locs] = findpeaks(-abs(dets), 'MinPeakProminence',0.0001);
    f_nat1     = locs(1) * df;
    omega1    = 2 * pi * f_nat1;
...
%% --------- Frequency sweep for GSa calculation ----------
...
for I = I_vals
    ...
    denominator = m1 * (-Omega.^2 + 2j * zeta * omega1 * Omega + omega1^2);
    GSY = -2 * E * J * numerator ./ denominator;
    GSa = abs(GSY) ./ (Omega.^2);     % Acceleration FRF
    % ---------- plot ----------
    plot(f_range, abs(GSa), 'LineWidth', 2, 'DisplayName', ['I = ' num2str(I) ' A']); 
      
end

xlabel('Frequency (Hz)');
ylabel('|G_{Sa}(j\Omega)| [N*s^2/m]');
title('Force–Acceleration FRF (Mode 1)');
